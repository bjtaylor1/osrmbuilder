...
function get_speed (way)
  ...
  local thespeed = -1
	...
	local specificspeed=tourist.get_specific_speed(way)
...
--special ways override everything else and we can go max (unclassified) speed on them
  ...
  if maxspeed ~= nil and maxspeed >= 112 and "roundabout" ~= junction then --112 = 70 mph
		print "motorway!"
    return -1
  end

  -- Set the avg speed on the way if it is accessible by road class
  if (tourist.speed_profile[highway] ~= nil ) then
      return tourist.speed_profile[highway]
	else
		return -1
  end
